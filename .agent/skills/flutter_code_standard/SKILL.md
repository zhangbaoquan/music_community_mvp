...
高质量的代码必须伴随高质量的注释，严禁出现“无注释”或“纯英文注释”的裸奔代码。

### 3.1 语言要求
- **全局中文**：所有代码注释（包括类注释、方法注释、行内解释）必须使用**规范的中文**。严禁使用全英文注释（除非复制的第三方开源库原文）。

### 3.2 类与文件级别注释 (Class-level)
- 每个非私有类（特别是 `View`, `Controller`, `Service`, `Model`）顶部必须有 `///` 格式的文档注释。
- **注释内容需包含**：类的核心职责、依赖关系或特殊逻辑流转。
```dart
/// 评论管理控制器 (CommentsController)
///
/// 负责处理文章评论的拉取、发布、删除逻辑。
/// 依赖 [ArticleService] 进行数据交互，由 GetX 负责生命周期管理。
class CommentsController extends GetxController {
```

### 3.3 方法与业务级别注释 (Method-level)
- 包含复杂业务逻辑的方法（如：涉及多表操作、状态机变更、复杂计算等）必须有 `///` 说明。
- 行内注释不要重复代码字面意思（What），而应解释**为什么这么做**（Why）和**整体逻辑意图**（How）。
```dart
// 正确 ✅ (解释 Why 和 How)
// 为避免频繁触发动画导致卡顿，这里在列表滚动时暂停预加载，并在停止滚动后恢复。

// 错误 ❌ (重复代码字面意思)
// 判断 isScrolling 是否为 true
// if (isScrolling == true) { ... }
```

...
- **必须使用中文**：所有代码注释必须使用规范的中文编写，严禁使用全英文注释（除非引用第三方库原文）。
- **类级别注释**：每个非私有 Class（尤其是 View、Controller、Service 和 Model）**顶部必须有** `///` 格式的文档注释，说明该类的职责和上下文。
- **方法级别注释**：复杂的业务方法、状态流转和公共 API **必须包含**逻辑说明，帮助阅读者迅速理解代码意图。
- **禁止无意义注释**：注释应解释 "Why"（为什么这么做）和 "How"（整体逻辑流转），而非简单重复代码的字面意思。
